 ...
 * collisions will be rare, but they will happen, so we deal with them using chaining. This code
...
private data class NodeStorageList<E: Any>(val hashValue: Int, val elements: List<E>): NodeStorage<E> {
    override fun equals(other: Any?): Boolean = when(other) {
        is NodeStorageOne<*> -> false
        is NodeStorageList<*> -> other.hashValue == this.hashValue && elements.all { other.elements.contains(it) }
        else -> false
    }
...
fun <E: Any> NodeStorage<E>.insert(element: E): NodeStorage<E> = when(this) {
...
fun <E: Any> nodeStorageOf(hashValue: Int, element: E, priorStorage: NodeStorage<E>? = null): NodeStorage<E> =
...
fun <E: Any> NodeStorage<E>.remove(element: E): NodeStorage<E>? = when(this) {
    is NodeStorageOne -> null
    is NodeStorageList -> if (elements.size > 2) {
        NodeStorageList(hashValue, elements.filter { it != element })
    } else {
        NodeStorageOne(hashValue, elements.first { it != element })
    }
}
